...
def collect_env_info():
    ...
    try:
        ...
        if has_cuda and getattr(_C, "has_cuda", lambda: True)():
            data.append(
                ("detectron2 arch flags", detect_compute_compatibility(
                    CUDA_HOME, _C.__file__))
            )

    data.append(get_env_module())
    data.append(("PyTorch", torch_version + " @" +
    ...
    if has_gpu:
        ...
        for k in range(torch.cuda.device_count()):
            cap = ".".join((str(x)
                           for x in torch.cuda.get_device_capability(k)))
        ...
        if has_rocm:
            msg = " - invalid!" if not (
                ROCM_HOME and os.path.isdir(ROCM_HOME)) else ""
            ...
            try:
                ...
                pass
            msg = " - invalid!" if not (
                CUDA_HOME and os.path.isdir(CUDA_HOME)) else ""
...
if __name__ == "__main__":
    try:
        from icm.models.feature_extractor.utils.collect_env import collect_env_info as f
