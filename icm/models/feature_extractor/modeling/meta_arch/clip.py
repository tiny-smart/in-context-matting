...
class ClipAdapter(nn.Module):
    def __init__(self, name="ViT-B-32", normalize=True):
        ...
        if os.path.exists(os.path.join("pretrained_models",name)):
            openai_clip = torch.load(os.path.join("pretrained_models",name,'openai_clip.pth'))
            preprocess = torch.load(os.path.join("pretrained_models",name,'preprocess.pth'))
        else:
            openai_clip, _, preprocess = open_clip.create_model_and_transforms(
                name, pretrained="openai"
            )
            os.makedirs(os.path.join("pretrained_models",name), exist_ok=True)
            torch.save(openai_clip, os.path.join("pretrained_models",name,'openai_clip.pth'))
            torch.save(preprocess, os.path.join("pretrained_models",name,'preprocess.pth'))
