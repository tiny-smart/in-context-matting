...
from icm.models.feature_extractor.checkpoint.odise_checkpointer import LdmCheckpointer
from icm.models.feature_extractor.modeling.meta_arch.clip import ClipAdapter
from icm.models.feature_extractor.utils.file_io import PathManager
...
def build_ldm_from_cfg(cfg_name) -> _LatentDiffusion:
    ...
    elif cfg_name.startswith("v2") or cfg_name.startswith("x4"):
...
class LatentDiffusion(nn.Module):
    ...
    def apply_model_with_guidence(self, x_noisy, t, cond):
        ...
        eps, rest = model_out[:,
                              : self.latent_dim], model_out[:, self.latent_dim:]
    ...
    def encode_to_latent(self, input_image):
        ...
        latent_image = self.ldm.get_first_stage_encoding(
            encoder_posterior.mean)
    ...
    def forward_test(self, batched_inputs):
        ...
        if uncond_inputs is None:
            ...
            latent_samples = self.diffusion.ddim_sample_loop(
                ...
                shape=(batch_size * 2, self.latent_dim,
                       *self.latent_image_size),
                # noise=noise,
                device=self.device,
                clip_denoised=False,  # no clip_denoised for LDM inference
                model_kwargs={"cond": torch.cat(
                    [cond_inputs, uncond_inputs], dim=0)},
            )[:batch_size]
    ...
    def __init__(
        ...
        if share_noise:
            ...
            self.register_buffer(
                "shared_noise",
                torch.randn(1, self.ldm.latent_dim, *
                            self.ldm.latent_image_size, generator=rng),
            )
        else:
            self.shared_noise = None

        # move to LdmImplicitCaptionerExtractor
    ...
    def reset_dim_stride(self):
        ...
        for i_level in range(self.ldm.encoder.num_resolutions):
            for i_block in range(self.ldm.encoder.num_res_blocks):
                all_encoder_blocks.append(
                    self.ldm.encoder.down[i_level].block[i_block])
        ...
        for idx, block in enumerate(self.ldm.unet.output_blocks):
            if idx in self.unet_block_indices:
                ...
                group_size = 3
                unet_strides.append(
                    64 // (2 ** ((idx + group_size) // group_size - 1)))
        ...
        for i_level in reversed(range(self.ldm.decoder.num_resolutions)):
            for i_block in range(self.ldm.decoder.num_res_blocks + 1):
                all_decoder_blocks.append(
                    self.ldm.decoder.up[i_level].block[i_block])
        ...
        for idx in self.decoder_block_indices:
            ...
            group_size = 3
            decoder_strides.append(
                8 // (2 ** ((idx + group_size) // group_size - 1)))
            decoder_blocks.append(all_decoder_blocks[idx])

        feature_dims = encoder_dims + unet_dims * \
            len(self.steps) + decoder_dims
        feature_strides = encoder_strides + \
            unet_strides * len(self.steps) + decoder_strides
    ...
    def grouped_indices(self):
        ...
        for i in range(len(self.unet_block_indices)):
            ...
            for t in range(len(self.steps)):
                cur_indices.append(
                    i + t * len(self.unet_block_indices) + offset)
    ...
    def forward(self, batched_inputs):
        ...
        for i, t in enumerate(self.steps):
            ...
            if t < 0:
                ...
                noisy_latent_image = self.ldm.diffusion.q_sample(
                    latent_image, t, noise)
...
class PositionalLinear(nn.Module):
    def __init__(self, in_features, out_features, seq_len=77, bias=True):
        ...
        self.positional_embedding = nn.Parameter(
            torch.zeros(1, seq_len, out_features))
