...
class CropResize(object):
    ...
    def __call__(self, sample):
        ...
        h, w = img.shape[:2]
        if h == w:
            img_crop =  cv2.resize(img, self.output_size, interpolation=maybe_random_interp(cv2.INTER_NEAREST))
            alpha_crop = cv2.resize(alpha, self.output_size, interpolation=maybe_random_interp(cv2.INTER_NEAREST))
            trimap_crop = cv2.resize(trimap, self.output_size, interpolation=cv2.INTER_NEAREST)
            mask_crop = cv2.resize(mask, self.output_size, interpolation=cv2.INTER_NEAREST)
        elif h > w:
            margin = (h-w)//2
            ...
            trimap_crop = cv2.resize(trimap, self.output_size, interpolation=cv2.INTER_NEAREST)
            mask_crop = cv2.resize(mask, self.output_size, interpolation=cv2.INTER_NEAREST)
        sample.update({'image': img_crop, 'alpha': alpha_crop, 'trimap': trimap_crop, 'mask': mask_crop})
        return sample
...
class MultiDataGeneratorDoubleSet(Dataset):
    ...
    def __init__(self, data, crop_size = 1024, phase="train"):
        ...
        val_trans = [ CropResize((self.crop_size, self.crop_size)), ToTensor(phase="val") ]
